...
class TemplateAlignmentTests(unittest.TestCase):
    ...
    def test_preset_layout_matches_a_locally_shifted_table_by_topology(self):
        source = np.full((500, 400), 255, np.uint8)
        target = np.full_like(source, 255)
        checkbox_positions = [
            (50 + column * 70, 60 + row * 70)
        ...
        self.assertTrue(result.accepted)
        self.assertTrue(result.compatible)
        self.assertEqual(result.matched_boxes, 15)
        for mapped, source_box in zip(result.config.fields[1].boxes, table_boxes):
            self.assertAlmostEqual(mapped.x, source_box.x + 28, delta=4)
            self.assertAlmostEqual(mapped.y, source_box.y, delta=3)

    def test_preset_layout_rejects_shifted_table_with_incompatible_topology(self):
        source = np.full((500, 400), 255, np.uint8)
        target = np.full_like(source, 255)
        checkbox_positions = [
            (50 + column * 70, 60 + row * 70)
